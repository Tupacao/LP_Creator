...
describe('AuthContext Component', () => {
    ...
    test('fetches user data on mount if userId exists in localStorage', async () => {
        ...
        await screen.findByText('User: {"id":"123","nome":"Teste","email":"teste@example.com","senha":"senha123"}');
    ...
    test('sets error state if fetching user data fails', async () => {
        localStorage.setItem('userId', '123');
        (userService.getUser as jest.Mock).mockRejectedValue(new Error('Erro ao buscar usuário'));
    ...
    test('login function updates state and navigates to /home on successful login', async () => {
        ...
        expect(authService.login).toHaveBeenCalledWith({ email: 'teste2@example.com', senha: 'senha456' });
    });
    ...
    test('register function registers user and navigates to /login', async () => {
        ...
        const RegisterComponent = () => {
            ...
            const handleClick = async () => {
                await register({ nome: 'Novo', email: 'novo@example.com', senha: 'senha' });
            };
        ...
        expect(authService.register).toHaveBeenCalledWith({ nome: 'Novo', email: 'novo@example.com', senha: 'senha' });
    });
